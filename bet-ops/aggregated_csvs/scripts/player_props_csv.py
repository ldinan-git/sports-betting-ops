...
if __name__ == '__main__':
    ...
    diff_cols = [col for col in agg_df.columns if '_diff' in col]
    all_diffs = agg_df_filtered[diff_cols].values.flatten()
    top_5_diffs = np.sort(all_diffs)[:50]

    rows = []
    for diff in top_5_diffs:
        for col in diff_cols:
            row_idx = np.where(agg_df[col].values == diff)
            if len(row_idx[0]) > 0:
                row = agg_df.iloc[row_idx[0]][['market', 'description', 'name', 'point'] + [c for c in agg_df.columns if '_price' in c]].values.tolist()[0]
                row.insert(0, col.split('_')[0])  # insert the column name at the beginning of the row
                rows.append(row)

    df = pd.DataFrame(rows, columns=['index', 'column_name', 'market', 'description', 'name', 'point'] + [c for c in agg_df.columns if '_price' in c])
